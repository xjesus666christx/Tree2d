...
TreeEditor.prototype.selectNode = function(name) {
	function nodeSearch(node, name) {
		if (node.field === name) return node;
		for (var i in node.childs) {
			if (node.childs[i].field) {
				var r = nodeSearch(node.childs[i], name);
				if (r) return r;
			}
		}
	}
	...
	if (this.node) {
		...
		var node = n1 = nodeSearch(this.node, name);
